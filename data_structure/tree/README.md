...
- <i id="2"></i>**`存储结构`**   
    对于存储结构,可能会联想到[顺序存储和链式存储结构](../0-define/linear_list#3)。但是对于数这种可能会有很多孩子的特殊数据结构，只用顺序存储结构或者链式存储结构很难实现，可以将这两者结合，产生主要的三种存储结构表示法:  
    - 双亲表示法  
        ![parent](./parent.webp)
        - 定义：  
        ...
        - 双亲表示法的特点
            ...
            2. 根据结点的parent真正很容易找到它的双亲结点。所用时间复杂度为O(1),直到parent为-1时，表示找到了树结点的根。
            3. 缺点: 如果要找到孩子结点，需要遍历整个结构才行。
            4. 增加一个结点最左边孩子的域(长子域)，这样就可以很容易得到结点的孩子。如果没有孩子的结点，这个长子域就设置为-1  
